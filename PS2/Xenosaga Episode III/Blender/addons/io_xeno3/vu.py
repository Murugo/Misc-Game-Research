...
class VifParser:
  ...
  def parse_until_run_cmd(self, buf, offs=0):
    '''Parses the given buffer until executing a command which executes the microprogram (e.g. MSCNT).'''
    while offs < len(buf):
      ...
      if cmd == 0b00000000:  # NOP
        ...
        self.cl = imm & 0xFF
        self.wl = (imm >> 8) & 0xFF
      elif cmd == 0b00010111:  # MSCNT
        # Run microprogram (stop parsing)
        return offs
        ...
        if vnvl == 0b0000:  # S-32
        ...
        elif vnvl == 0b1110:  # V4-8
          width = 4
          for i in range(qwd):
            val = [_DEF_WORD, _DEF_WORD, _DEF_WORD, _DEF_WORD]
            if self.cl >= self.wl or (i % self.wl) < self.cl:
              if usn:
                val = self._getnpackedu8(buf, width * j + offs, 4)
              else:
                val = self._getnpacked8(buf, width * j + offs, 4)
              j += 1
            addroffs = self.cl * (i // self.wl) + (i %
                                                   self.wl) if self.cl >= self.wl else 0
            self.vumem[addr + addroffs] = [
          ...
          raise VuParseError(
              f'Unsupported unpack vnvl {hex(vnvl)} at offset {hex(offs - 4)}')
        offs += j * width
        if (offs % 4) > 0:
          offs += 4 - (offs % 4)
      else:
        raise VuParseError(
            f'Unrecognized vifcmd {hex(cmd)} at offset {hex(offs - 4)}')
    return offs
  ...
  def _getnpacked8(self, b, offs, n):
    values = struct.unpack('<' + 'b' * n, b[offs:offs+n])
    return [struct.pack('<i', v) for v in values]
  
  def _getnpackedu8(self, b, offs, n):
    values = struct.unpack('<' + 'B' * n, b[offs:offs+n])
    return [struct.pack('<i', v) for v in values]
